...
/* int functionType */
#define beginDataAcq		0
#define readDataToPlot		1
#define showAirplaneModel	2
...
int panelHdl;
int panelHdl2; //For the airplane model
...
void runFuncInAnotherThread(int functionType);
void initVars(void);
...
void runFuncInAnotherThread(int functionType){
	switch(functionType){
		case beginDataAcq:
			...
			break;
		case showAirplaneModel:
			CmtScheduleThreadPoolFunction (poolHandle, showAirplaneModelll, NULL, NULL);
			break;
	}
}
...
void startDataAcq(void){
	...
	receiveFlag = 1;
	runFuncInAnotherThread(beginDataAcq);
}
void pauseDataAcq(void){
	receiveFlag = 0;
...
int CVICALLBACK YesConfigBtn(int panel, int control, int event,
		void *callbackData, int eventData1, int eventData2) {
	switch(event){
		case EVENT_COMMIT:
			configComFlag = 0;
...
int CVICALLBACK CancelConfigBtn(int panel, int control, int event,
		void *callbackData, int eventData1, int eventData2) {
	switch(event){
		case EVENT_COMMIT:
			DiscardPanel(panel);
